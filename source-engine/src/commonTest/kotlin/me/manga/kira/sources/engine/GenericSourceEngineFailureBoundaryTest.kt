...
class GenericSourceEngineFailureBoundaryTest {
    ...
    @Test
    fun pages_reads_headers_once_without_granting_authority_to_an_untrusted_live_base() = runTest {
        val fixture = Fixture()

        assertEquals(
            SourceEngineResult.Success(listOf(SourcePage("$LIVE_BASE_URL/page/1.webp", emptyMap()))),
            fixture.engine.pages(MANGA, CHAPTER),
        )
        assertEquals(listOf(Provider.BASE_URL, Provider.HEADERS), fixture.providerCalls)
        assertEquals(SourceRequest(CHAPTER.url, headers = emptyMap()), fixture.requests.single())
    }

    @Test
    fun pages_reuses_one_header_read_for_the_request_and_returned_images() = runTest {
        val fixture = Fixture(config = CONFIG.copy(trustedHosts = listOf("live.example")))
