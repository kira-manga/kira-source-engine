#!/usr/bin/env python3
"""App61: one strict unsigned bundle, then one same-cache checksum-negative help."""
import base64
import ctypes
import hashlib
import json
...
require(os.environ.get("GITHUB_REF") == "refs/heads/validation/app61-native-consume-20260915-01", "Wrong branch")
...
require(binding["schema"] == "app61-strict-consume-v1" and binding["status"] == "PRIMARY_BOUND_FOR_CONSUMPTION"
        and binding["authorization"] == "APP61_ONE_STRICT_BUNDLE_AND_CHECKSUM_NEGATIVE_AUTHORIZED",
...
result = {"status": "NOT_RUN", "nonshipping": True, "errors": [], "commands": [],
          ...
          "scope": [":app:bundleRelease"], "negative_scope": ["help", "--offline"], "batches": [],
...
def alter_consumed_agp():
    ...
    proof = {"artifact": str(artifact.relative_to(gradle_home)), "coordinate": "com.android.tools.build:gradle:9.2.1",
        ...
        "fresh_owned_cache": True, "positive_bundle_completed": True, "changed": False}
...
gradle.taskGraph.whenReady { graph ->
    if (graph.allTasks.any { task -> task.name.toLowerCase(java.util.Locale.ROOT) =~ /upload|publish/ }) {
        throw new GradleException("App61 NONSHIPPING forbids all upload/publication tasks")
    }
}
''')
    ...
    last_batch_settled = False
    try:
        # Primary-approved safety exclusion: only the external-upload finalizer, not compile/R8/lint.
        result["gradle_exit"] = command(["./gradlew", *result["scope"], *arguments,
            "-x", ":app:uploadCrashlyticsMappingFileRelease", "-I", str(upload_guard)],
            "strict-bundle", app, 38 * 60, consume_env)
    finally:
        consume_env.clear()
        positive_settled = settle_batch("positive")
        last_batch_settled = True
    require(result["gradle_exit"] == 0 and positive_settled and not result["errors"], "Strict positive bundle or its cleanup failed")
    ...
    try:
        result["negative_exit"] = command(["./gradlew", *result["negative_scope"], *arguments,
            "-Dorg.gradle.dependency.verification.console=verbose"], "checksum-negative", app, 180)
    finally:
    ...
    passed = (result["gradle_exit"] == 0 and result.get("checksum_negative_qualified") is True
        and result.get("toolchain_inputs_verified") is True and result.get("bundle", {}).get("unsigned") is True
    ...
    result["status"] = "STRICT_CONSUMPTION_REVIEW_REQUIRED" if passed else "PARTIAL_OR_NOT_RUN"
    ...
    result["coverage"] = ("One source-native verifier, strict unsigned Android bundle and same-cache offline help checksum negative. "
...
sys.exit(result["gradle_exit"] if result["gradle_exit"] not in (None, 0) else
         (0 if result["status"] == "STRICT_CONSUMPTION_REVIEW_REQUIRED" else 1))
