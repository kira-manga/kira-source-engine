...
`d2617c00b54c832f427c88b9cec74bbd65ef6089`, verifies its inputs, and copies the
...
All twenty Gradle inputs then match the pinned derivative manifest, as do its four
wrapper inputs; the derivative updates only the reviewed app-script input digest. Publicd261 includes later application changes: this is **not whole-tree
...
Only `:app:lintVitalAnalyzeRelease`, `:app:dependencies --configuration releaseRuntimeClasspath`,
`:buildEnvironment`, and `:app:buildEnvironment` run once. This verifies the newly inlined
applied-script lint boundary and obtains separately labelled native graph bootstrap, not a
release bundle or complete graph. The earlier bundle/lint failure remains evidence; no R8,
packaging, test or unchanged Native16 replay is requested. Metadata generation still scans
resolvable configurations as documented by Gradle; check unresolved nodes and emitted locks.
Strict consumption and release packaging remain separate future qualification after trusted
inputs exist. Native SHA256/lock generation, no build/configuration cache, no Maven-local
or composite override, exact launcher, installed SDK37.0 and auto-download refusals are unchanged.
The corrected reviewed nine-file overlay includes the separate exact provider selector.
...
The manifest overlay is a reviewed derivative: its app/build.gradle.kts graph-input
digest binds the exact inlined helper source. The original nine-file scaffold and
its historical manifests remain unchanged; this one digest is not falsely attributed
to the original scaffold freeze.
